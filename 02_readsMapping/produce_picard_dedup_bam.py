...
def produce_cmd(ref, groupbams, outmetrics, outbam, picard, tmpdir, mem, samtools):
...
def main(ref, outdir, picard, tmpdir, mem, samtools, inbams):
    ...
    for ngroup, (smid, groupbams) in enumerate(groups.items(), 1):
        ...
        outbam = os.path.join(outdir, f'{smid}.dedup.bam')
        outmetrics = os.path.join(outdir, f'{smid}.marked_dup_metrics.txt')
        cmd = produce_cmd(ref, groupbams, outmetrics, outbam, picard, tmpdir, mem, samtools)
